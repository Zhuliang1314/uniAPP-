<template>
	<view class="page">
		...
		:showRight="true" :showRightIcon="false">
			<free-main-button v-if="item.status === 'pending'" slot="right" name="同意" @click="handle(item)"></free-main-button>
			<text v-else slot="right" class="text-muted font-sm">{{ item | formatTitle }}</text>
		</free-list-item>
		<!-- 上拉加载 -->
