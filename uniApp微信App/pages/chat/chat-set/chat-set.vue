<template>
	<view style="background-color: #EDEDED;">
		...
		<view class="flex flex-wrap py-3 bg-white">
			...
			<view v-if="detail.chat_type === 'user'" class="flex flex-column align-center justify-center mb-2"
				style="width: 150rpx;">
			...
			<view class="flex flex-column align-center justify-center mb-2" style="width: 150rpx;">
				<view class="flex align-center justify-center border" style="width: 120rpx;height: 120rpx;"
					hover-class="bg-light" @click="openMail">
		...
		<free-list-item title="强提醒" showRight :showLeftIcon="false" :showRightIcon="false">
			<switch slot="right" :checked="detail.strongwarn" color="#08C060"
				@change="updateChatItem($event,'strongwarn')" />
		</free-list-item>
		...
		<view v-if="detail.chat_type === 'group'">
			...
			<free-list-item title="显示群成员昵称" showRight :showLeftIcon="false" :showRightIcon="false">
				<switch slot="right" :checked="detail.shownickname" color="#08C060"
					@change="updateChatItem($event,'shownickname')" />
			</free-list-item>
		</view>
...
<script>
	...
	export default {
		...
		methods: {
			openMail() {
				uni.navigateTo({
					url: '../../mail/mail/mail?type=createGroup',
				});
			},
			updateName() {
